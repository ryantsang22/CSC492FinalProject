...
def onepixel_perturbation_image(attack, orig_x, sigma):
  ...
  for counter in range(orig_x.shape[0]):
      for counter2 in range(orig_x.shape[1]):
        batch_x[(counter*orig_x.shape[1]+counter2)*n_corners:(counter*orig_x.shape[1]+counter2+1)*n_corners] = np.clip(onepixel_perturbation(attack, orig_x, [counter,counter2], sigma), 0.0, 1.0)
...
class CSattack():
  ...
  def perturb(self, x_nat, y_nat):
    ...
    # The attack was built for MNIST and CIFAR-10, so we must change the number of classes to 2 from 10
    self.n_classes = 2
